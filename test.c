...
static void
test(struct siphash *s, const unsigned char *buf, int len, const uint64_t *e)
{
    if (!len) {
        test_total++;
        unsigned char output[16];
        siphash128_final(s, output);
        ...
        for (int i = len - 16 > 1 ? len - 16 : 1; i <= len; i++) {
            struct siphash c = *s;
            siphash128_update(&c, buf, i);
            test(&c, buf + i, len - i, e);
        }
    }
}
...
static const uint64_t expect[64][2] = {
